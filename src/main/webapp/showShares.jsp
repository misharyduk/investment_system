    ...
    <section id="hero_two" class="d-flex align-items-center">
        <div class="container position-relative" data-aos="fade-up" data-aos-delay="100">
            ...
            <div class="row icon-boxes">
                <c:forEach items="${shares.elements}" var="share" varStatus="loop">
                <div class="col-md-6 col-lg-3 d-flex align-items-stretch mb-5 mb-lg-0" data-aos="zoom-in"
                     data-aos-delay="${loop.index}00">
                    <div class="icon-box">
                        <div class="icon"><i class="${icons[loop.index]}"></i></div>
                        <h4 class="title"><a href="" data-bs-toggle="modal" data-bs-target="#exampleModalCenter" data-bs-id="${share.id}">
                            ${share.name}
                        </a></h4>
                        <p class="description">Price: ${share.price}. <br />Number of owners: ${share.ownershipHistory.size()}.</p>
                    </div>
                </div>
                </c:forEach>
            </div>
            <br />
                <p class="text-success text-center"><c:if test="${success == 1}">Share successfully confirmed</c:if></p>
                <p class="text-danger text-center"><c:if test="${error == 0}">Share cannot be confirmed</c:if></p>
            <c:if test="${shares.totalPages > 0}">
            <div class="h-100 d-flex align-items-center justify-content-center">
                <nav>
                    <ul class="pagination">
                        <li class="page-item ${shares.page == 1 ? 'disabled' : ''}">
                            <a class="page-link" href="?page=${shares.page < 1 ? 0 : shares.page - 1}" tabindex="-1">Previous</a>
                        </li>

                           <c:set var="num_page" value="${shares.page}" />
                           <c:forEach begin="${shares.page}"
                               end="${shares.totalPages >= shares.page+4 ? num_page+3 : shares.totalPages}"
                               step="1" var="i">

                                   <li class="page-item ${i == shares.page ? 'active' : ''}">
                                       <c:choose>
                                       <c:when test="${i == shares.page}">
                        ...
                        <c:if test="${shares.page + 4 <= shares.totalPages}">
                            <li class="page-item disabled">
                                <a class="page-link" href="#">...</a>
                            </li>
                        </c:if>

                        <li class="page-item ${shares.page == shares.totalPages ? 'disabled' : ''}">
                            <a class="page-link" href="?page=${shares.page == shares.totalPages ? 1 : shares.page + 1}">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">Do you really want to buy this share? <br />Fill the form</p>
                                ...
                                <form action="investments/add" id="submit_form_modal" method="post" role="form" class="php-email-form">
                                    <div class="row gy-2 gx-md-3">
                                        ...
                                        <div class="col-md-12 form-group">
                                            <label for="investmentAmount" class="col-sm-6 col-form-label">Re-enter sum:</label>
                                            <input type="text" name="investmentAmount" class="form-control" id="investmentAmount" placeholder="Amount" required />
                                        </div>
                                        <div class="form-group col-12">
                                            <label for="term" class="col-sm-6 col-form-label">Term:</label>
...
exampleModal.addEventListener('show.bs.modal', function (event) {
  ...
  modalTitle.textContent = 'Submit purchase for Share ' + id
  modalBodyInput.value = id

  document.getElementById('clientId').value = '1';
  document.getElementById('investmentType').value = 'share';
})
