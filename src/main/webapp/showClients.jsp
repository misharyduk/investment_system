...
<main id="main">
    ...
    <!-- ======= Services Section ======= -->
    <section id="services" class="services section-bg" style="margin-top: 6em; background-color: white;">
        <div class="container" data-aos="fade-up">

            <div class="section-title">
                <h2>Clients</h2>
                <p>Here you can observe all clients registered in the system. <strong>Total number: ${clients.elements.size()}</strong>. Use pagination to see another page. Clink on client to see his details</p>
            </div>

            <c:set var="vectors" value="${[
            ...
            <c:set var="colors" value="${['iconbox-blue', 'iconbox-orange', 'iconbox-pink', 'iconbox-yellow', 'iconbox-red', 'iconbox-teal']}" />

            <div class="row">
                <c:forEach items="${clients.elements}" var="client" varStatus="loop">
                <div class="col-lg-4 col-md-6 d-flex align-items-stretch" data-aos="zoom-in" data-aos-delay="${loop.index}00">
                    <div class="icon-box ${colors[loop.index]}">
                        <div class="icon">
                            <svg width="100" height="100" viewBox="0 0 600 600" xmlns="http://www.w3.org/2000/svg">
                                <path stroke="none" stroke-width="0" fill="#f5f5f5" d="${vectors[loop.index]}"></path>
                            </svg>
                            <i class="ri-user-line"></i>
                        </div>
                        <h4><a href="clients/${client.id}">${client.factory.name}</a></h4>
                        <p>${client.investments.size()} investments. Address: ${client.factory.address}</p>
                    </div>
                </div>
                </c:forEach>
<!--
...
-->
            ...
            <c:if test="${clients.totalPages > 0}">
            <div class="h-100 d-flex align-items-center justify-content-center">
                <nav>
                    <ul class="pagination">
                        <li class="page-item ${clients.page == 1 ? 'disabled' : ''}">
                            <a class="page-link" href="?page=${clients.page < 1 ? 0 : clients.page - 1}" tabindex="-1">Previous</a>
                        </li>

                           <c:set var="num_page" value="${clients.page}" />
                           <c:forEach begin="${clients.page}"
                               end="${clients.totalPages >= clients.page+4 ? num_page+3 : clients.totalPages}"
                               step="1" var="i">

                                   <li class="page-item ${i == clients.page ? 'active' : ''}">
                                       <c:choose>
                        ...
                        <c:if test="${clients.page + 4 <= clients.totalPages}">
                            <li class="page-item disabled">
                                <a class="page-link" href="#">...</a>
                            </li>
                        </c:if>

                        <li class="page-item ${clients.page == clients.totalPages ? 'disabled' : ''}">
                            <a class="page-link" href="?page=${clients.page == clients.totalPages ? 1 : clients.page + 1}">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
